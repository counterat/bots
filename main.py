...
async def distribute_other_chats(ids_of_chats):
    ...
    while True:
        ...
        template_my = '''
**💵📣ЗАКАЗАЛ РАССЫЛКУ НО ТЕБЕ ТАК НИКТО И НЕ НАПИСАЛ?🤥😩

💹ТЕБЕ К НАМ😏**

**💯НАШИ ПРЕИМУЩЕСТВА😎**

__💾 Ежеминутно пополняющаяся БАЗА из более 500 РЕАЛЬНЫХ ЧАТОВ💬
...
За подробностями: @arend_avit0  
        ...
        for id_of_chat in ids_of_chats:
            try:
                ...
                await app.send_message(chat_id=id_of_chat, text=template_my, parse_mode=ParseMode.MARKDOWN)
                print(id_of_chat)
                messages_sent += 1
